...
local function getMaxMinDefaultTorqueCurves()
	local graph_file = io.open("vehicles/bug/lua/hrb/data/dynamic_torque_graph.csv")
...
local function playerBehaviourValuation()
	...
	elseif curBrokenBeams > 35 then
		if hardCrashCounter > 4 then
			playHornSound("SFX_bug_sadhorn2", 50, 0.8)
		end
	...
	elseif curBrokenBeams < 18 then
		...
		if hardCrashCounter > 4 then
			playHornSound("SFX_bug_sadhorn2",50, 1 - 0.0005 * math.abs((playerTrust - playerTrustMax)))
		end
...
local function onInit(jbeamData)
	...
	for _,part in pairs(v.data.activePartsData) do
		if part.partName == "bug_hrb_personality" then enabled = true end
		if part.partName == "bug_engine_1.8_hrb" then herbieEngine = true end
	end
	if not enabled then return end
...
-- Enter "hrb.reset()" and hit execute.
