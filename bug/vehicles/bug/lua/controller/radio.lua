...
function SoundObj:load(filepath, loop)
	...
	if nodeId then
		...
		self.sound = obj:createSFXSource(filepath, sfxDescription, name, nodeId or 0)
		-- Note:
	...
	if self.sound then
		obj:playSFX(self.sound)
		self:setVolume(self.volume)
	end
end

function SoundObj:has()
	return self.sound ~= nil
end

function SoundObj:unload()
	if self.sound == nil then return end
	self:stop()
	obj:deleteSFXSource(self.sound)
	self.sound = nil
end

function SoundObj:stop()
	if self.sound == nil then return end
	obj:stopSFX(self.sound)
end

function SoundObj:play()
	if self.sound == nil then return end
	if not obj:isPlayingSFX(self.sound) then obj:playSFX(self.sound) end
end
...
function SoundObj:setVolume(volume)
	...
	self.volume = volume
	obj:setVolume(self.sound, volume)
end
...
function Station:new(freq, name, shuffle)
	...
	self.speaker = SoundObj:new(NodeOutput, 0)	-- Muted by default 
...
function Radio:new()
	...
	self.volume = 0
	self.volume2D = 1.8							-- Default volume for 2D mode. Use "Music slider" in game options to adjust.
	self.volume3D = 1.4							-- Default volume reduced by 40% in 3D mode as it is a lot louder compared to 2D.
...
function Radio:initalize(jbeam)
	...
	self.installed = true
	self.power = electrics.values.ignitionLevel > 0 and electrics.values.powerAvailable > 0
...
function Radio:playKnobSFX(set)
	playSound("event:>Vehicle>Interior>Handbrake_Ratchet>Ratchet_05_Ratchet", NodeSFX, 0.15, 0.8)
	-- Alternative SFX:
	--playSound("event:>Vehicle>Interior>Handbrake_Ratchet>Ratchet_03_Ratchet", NodeSFX)
end

function Radio:playPresetSFX(set)
	playSound(not set and SFXPresetUse or SFXPresetSet, NodeSFX)
end

function Radio:playStateSFX()
	playSound(self.state and SFXRadioOn or SFXRadioOff, NodeSFX)
end
...
function Radio:updateVolume(volume)
	...
	-- static SFX volume ...
	local noise = volume > 0 and match < 1
	if not noise then
	...
	if not self.speaker:has() then
		self.speaker:load(SFXRadioNoise, true) 
	end
...
function Radio:updateRadio(dt)
	...
	self.power = electrics.values.ignitionLevel > 0 and electrics.values.powerAvailable > 0
	if not self.power and lastPower == self.power then return end
...
local function onInit(jbeamData)
	radio = radio or Radio:new()
	radio:initalize(jbeamData)
end
...
local function volumeSet(volume) radio:setVolume(volume) end
...
-- Public Interface
-- Console: controller.getController('radio').resetData()
M.init         = onInit
M.reset        = onReset
